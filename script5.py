                        ...
                        for lIndex in range(len(lines)):
                            ...
                            if oline.startswith(OBJECTSTART):
                                ...
                                if id not in objects:
                                    extent = { "plate" : plate, "X1": 999.0, "Y1": 999.0, "X2": 0.0, "Y2": 0.0}
...
while remaining_flush > 10 and objects_left and count < 10:
    ...
    if highest_flush > 100:
        ...
        print("Remaining flush saving too small - finishing object selection")
        objects_left= False
...
print("now go through selections and create modifled project 3mf files")
...
for filename,objects in files.items():
    ...
    for object_id, object in objects.items():
        if "Selected" in object:
            bSelected = True
            print(filename,object_id,object["extent"])
    if bSelected:
                            ...
                            for node in root.findall(f"./{ns}resources/{ns}object"):
                                ...
                                for node2 in node.findall(f"./{ns}components/{ns}component"):
                                    ...
                                    for item in root.findall(f"./{ns}build/{ns}item[@objectid='{id}']"):
                                        ...
                                        transform2 = item.attrib['transform']
                                        #print("transform2:",transform2) 
                                        tfm_split = transform2.split(" ")
                                        x = float(tfm_split[9])
                                        y = float(tfm_split[10])
                                        print("x,y:",x,y)
                                        matched = 0
                                        for object_id, object in objects.items():
                                            if "Selected" in object:
                                                if x > object["extent"]["X1"] and x < object["extent"]["X2"]:
                                                    if y > object["extent"]["Y1"] and y < object["extent"]["Y2"]:
                                                        print("Match -gcode", object_id, 'to 3mf',id)
                                                        matched += 1
                                        if matched == 0:
                                            print("***Match not found - set to non printable")
